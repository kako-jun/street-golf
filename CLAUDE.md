...
| キー | フェーズ | 効果 |
|---|---|---|
| `↑` / `↓` | ParSelect | 選択中の Par を上下に移動 |
| `3` / `4` / `5` | ParSelect | Par 3 / 4 / 5 を直接指定して Playing に遷移 |
| `Enter` / `Space` | ParSelect | 現在選択中の Par を確定して Playing に遷移 |
| `1`-`8` | Playing | クラブ選択（Driver / 3I / 5I / 7I / 9I / PW / SW / Putter） |
| `←` / `→` | Playing | yaw（水平方向）を ±約 2° |
| `↑` / `↓` | Playing | pitch（打ち出し角）を ±2°（0°-45° クランプ） |
